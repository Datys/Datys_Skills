...
public class Location
{
	...
	public readonly global::Location location;
	...
	private static void AddLocationToZoneSystem(ZoneSystem __instance)
	{
		...
		foreach (Location location in registeredLocations.Where(l => !l.replace))
		{
		...
		foreach (Location location in registeredLocations.Where(l => l.replace))
		{
			var findExisting = __instance.m_locations.First(l => l.m_prefabName == location.location.gameObject.name);
			if (findExisting == null) continue;
			findExisting.m_prefab = softReferences[location];
		}
	}
